"""Mirror of upstream ``Projection_MultiView_Hy2.0_Qwen_2Views_LowPoly_FullWorkflow.json``.
...
def main() -> int:
    parser = argparse.ArgumentParser(
        prog='projection_multiview_hy20_qwen_2views_lowpoly_full', description=__doc__,
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    add_common_args(parser, 'Projection_MultiView_Hy20_Qwen_2Views_LowPoly_FullWorkflow.glb')
    parser.add_argument('--target-faces', type=int, default=5000, help='Decimation target')
    return not_implemented_stub(
        parser, workflow_json='Projection_MultiView_Hy2.0_Qwen_2Views_LowPoly_FullWorkflow.json',
        missing_features=['MultiViewTexturing pipeline', 'Hunyuan3D + Qwen full workflow'],
