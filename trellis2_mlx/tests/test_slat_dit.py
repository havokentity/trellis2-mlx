...
def slat_dit_state() -> dict[str, np.ndarray]:
    if not _SLAT_SHAPE_DIT.exists():
        pytest.skip(f"SLAT DiT weights not found at {_SLAT_SHAPE_DIT}")
    # Header-driven load: read every key from the file as fp32 numpy.
    import struct

    with open(_SLAT_SHAPE_DIT, "rb") as f:
        n = struct.unpack("<Q", f.read(8))[0]
        header = __import__("json").loads(f.read(n).decode())
    header.pop("__metadata__", None)
    keys = list(header.keys())
    raw = _safetensors_load_keys(_SLAT_SHAPE_DIT, keys)
