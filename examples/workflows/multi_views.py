"""Mirror of upstream ``MultiViews.json``.
...
def main() -> int:
    parser = argparse.ArgumentParser(
        prog='multi_views', description=__doc__,
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    add_common_args(parser, 'MultiViews.glb')
    parser.add_argument('--images', type=str, default=None, help='Comma-separated list of input view images')
    ...
    return not_implemented_stub(
        parser, workflow_json='MultiViews.json',
        missing_features=['MultiView generator (Trellis2MeshWithVoxelMultiViewGenerator) — image-to-3D from multiple input views', 'UV unwrap + rasterizer'],
        upstream_nodes=['Trellis2MeshWithVoxelMultiViewGenerator', 'Trellis2UnWrapAndRasterizer'],
        notes='',
    )
