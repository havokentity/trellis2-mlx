"""Mirror of upstream ``Projection_MultiView_Hy2.0_Qwen_2Views_Upscaled.json``.
...
def main() -> int:
    parser = argparse.ArgumentParser(
        prog='projection_multiview_hy20_qwen_2views_upscaled', description=__doc__,
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    add_common_args(parser, 'Projection_MultiView_Hy20_Qwen_2Views_Upscaled.glb')
    parser.add_argument('--texture-resolution', type=int, default=4096, help='UV atlas size')
    return not_implemented_stub(
        parser, workflow_json='Projection_MultiView_Hy2.0_Qwen_2Views_Upscaled.json',
        missing_features=['MultiViewTexturing pipeline (upscaled)', 'Hunyuan3D + Qwen'],
